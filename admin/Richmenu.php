...
final class Richmenu
{
    ...
    protected function do_init(): void
    {
        ...
        add_filter('manage_' . Main::POSTTYPE_RICHERMENU . '_posts_columns', [$this, 'add_columns']);
        add_filter('manage_' . Main::POSTTYPE_RICHERMENU . '_posts_custom_column', [$this, 'show_columns'], 10, 2);
        add_action('save_post_' . Main::POSTTYPE_RICHERMENU, [$this, 'save_date'], 10, 2);
    }

    public function skip_quick_edit($enabled, $post_type)
    {
        if ($post_type === Main::POSTTYPE_RICHERMENU) {
            return false;
        }
    ...
    {
        if ($post->post_type !== Main::POSTTYPE_RICHERMENU) {
            return $post_states;
        }

        if ($post->ID == Main::get_option('richmenu_default')) {
