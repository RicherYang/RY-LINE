...
import './richmenu.scss';

$(function () {
    const $svg = $('#richmenu-image-svg');
    const $gLines = $svg.find('.area-lines');
    const $gTexts = $svg.find('.area-texts');
    const $areaTable = $('#richmenu-area-list tbody');
    const areaTemplate = wp.template('area-settings');
    const $actionTable = $('#richmenu-action-list tbody');
    const actionTemplate = wp.template('action-settings');

    if ($svg !== undefined) {
        const bgImageUrl = $svg.data('bg');
        const img = new Image();
        img.onload = function () {
            $svg.data('ratio', $svg.parent().width() / parseInt($svg.data('width')));
            ...
            $svg.find('#bg-pattern').attr({
                width: svgWidth,
                height: svgHeight
            });
            $svg.find('#bg-pattern image').attr({
                width: svgWidth,
                height: svgHeight,
                href: bgImageUrl
            });
    ...
    const getAreasData = () => {
        $.ajax({
            url: ajaxurl + '?action=ry-line/get-richmenu-areas',
            method: 'POST',
            dataType: 'json',
            data: {
                post_id: $('#post_ID').val(),
                _ajax_nonce: RYLineRichmenu.nonce.get
            }
        }).done(function (Jdata) {
            if (Jdata.success) {
    ...
    $('.ry-save-position').on('click', function (e) {
        ...
        let $btn = $(this);
        $.ajax({
            url: ajaxurl + '?action=ry-line/save-richmenu-areas',
            method: 'POST',
            dataType: 'json',
            data: {
                areas: decodeURIComponent($('#richmenu-area-list').serialize()),
                post_id: $('#post_ID').val(),
                _ajax_nonce: RYLineRichmenu.nonce.areas
            }
        }).done(function (Jdata) {
            if (Jdata.success) {
    ...
    $('.ry-save-action').on('click', function (e) {
        ...
        let $btn = $(this);
        $.ajax({
            url: ajaxurl + '?action=ry-line/save-richmenu-actions',
            method: 'POST',
            dataType: 'json',
            data: {
                actions: decodeURIComponent($('#richmenu-action-list').serialize()),
                post_id: $('#post_ID').val(),
                _ajax_nonce: RYLineRichmenu.nonce.actions
            }
        }).done(function (Jdata) {
            if (Jdata.success) {
                if (Array.isArray(Jdata.data) && Jdata.data.length) {
    ...
    $('.ry-create-menu').on('click', function (e) {
        ...
        $.ajax({
            ...
            data: {
                post_id: $('#post_ID').val(),
                _ajax_nonce: RYLineRichmenu.nonce.create
            }
        }).done(function (Jdata) {
            if (Jdata.success) {
                location.reload();
    ...
    $('.ry-set-alias').on('click', function (e) {
        ...
        $.ajax({
            ...
            data: {
                ...
                _ajax_nonce: RYLineRichmenu.nonce.alias
            }
        }).done(function (Jdata) {
            if (Jdata.success) {
    ...
    $('.ry-set-test').on('click', function (e) {
        ...
        $.ajax({
            ...
            data: {
                post_id: $('#post_ID').val(),
                _ajax_nonce: RYLineRichmenu.nonce.test
            }
        }).done(function (Jdata) {
            if (Jdata.success) {
                return;
    ...
    $('.ry-default-menu').on('click', function (e) {
        ...
        $.ajax({
            ...
            data: {
                post_id: $('#post_ID').val(),
                _ajax_nonce: RYLineRichmenu.nonce.default
            }
        }).done(function (Jdata) {
            if (Jdata.success) {
                $btn.text(Jdata.data);
    ...
    $('.ry-delete-menu').on('click', function (e) {
        ...
        $.ajax({
            ...
            data: {
                post_id: $('#post_ID').val(),
                _ajax_nonce: RYLineRichmenu.nonce.delete
            }
        }).done(function (Jdata) {
            if (Jdata.success) {
                location.reload();
